...
video=cv.VideoCapture('../picture/bikes.mp4')
...
while(True):
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            print(box)
            w,h=x2-x1,y2-y1
            if box.conf[0]*100>60:
                cvzone.cornerRect(read,(x1,y1,w,h))
                cvzone.putTextRect(read,f'{classNames[int(box.cls[0])]} {int(box.conf[0]*100)}%',(max(0,x1),max(35,y1)),scale=2,thickness=3,offset=10)
